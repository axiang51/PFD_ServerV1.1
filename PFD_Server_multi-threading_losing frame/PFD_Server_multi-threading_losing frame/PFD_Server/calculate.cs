...
namespace HST_Server
{
    ...
    public class ParamsCalculate
    {
        ...
        List<Human> HumanAux = new List<Human>();//用于前后两个统计时间间隔内数据的更替
        List<Human> AllHumans = new List<Human>();//存放所有摄像头检测出的人员数据
        ...
        public bool calculate(SHstPipeInfo sPipeInfo)  //进行计算的主要函数，数据输入格式为管道接收数据的格式
        {
            ...
            for (int i = 0; i < sPipeInfo.iVideoCount; i++)//Debug 第i个摄像头  修改为摄像头的实际数目IVideoCount，该数目为固定的数字10，不表示实际开启的摄像头路数
            {
                ...
                for (; j < sPipeInfo.szVideoInfo[i].iObjCount; j++)//第j个轨迹点
                {
                    ...
                    {
                        int ahs = 0;
                        for(; ahs < AllHumans.Count(); ahs++ )
                        {
                            if( (AllHumans[ahs].camid == sPipeInfo.szVideoInfo[i].iVideoID) && (AllHumans[ahs].id == sPipeInfo.szVideoInfo[i].szObjInfo[j].ID) )
                            {
                                point.x = sPipeInfo.szVideoInfo[i].szObjInfo[j].center_x;
                                point.y = sPipeInfo.szVideoInfo[i].szObjInfo[j].center_y;
                                AllHumans[ahs].points.Add(point);  //Debug 添加当前行人轨迹点
                                ...
                                human.flag = AllHumans[ahs].flag;   //由行人信息存储结构中取出当前人的信息
                                human.direction = AllHumans[ahs].direction;
                                human.speed = AllHumans[ahs].speed;
                                human.points = AllHumans[ahs].points;  //Debug

                                //2.方向确认
                                //位于此处原因：满足指定点数要求的轨迹可按照流程执行到此处，达到指定的点数需要多帧数据的累积
                                if (AllHumans[ahs].points.Count() >= 20)  //Debug  允许进行方向确认的轨迹点数需要的最小数值
                                {
                                    if (-1 == AllHumans[ahs].direction)  //表明此前未进行过方向判别，首次执行
                                    {
                                        if (AllHumans[ahs].points[0].y - AllHumans[ahs].points[AllHumans[ahs].points.Count() - 1].y >= 0)
                                        {
                                    ...
                                    AllHumans[ahs] = human;              //当前行人信息更新  direction
                                }

                                //3.人数统计
                                if (AllHumans[ahs].points.Count() >= 20)  //Debug 进行人数统计的条件
                                {
                                    if (-1 == human.flag)             //此前该轨迹未进行数量统计
                                    {
                                        ...
                                        AllHumans[ahs] = human;          //当前行人信息更新  flag
                                ...
                                speed.id = -1;
                                speed.speed = -1;
                                if (AllHumans[ahs].points.Count() >= 20)  //Debug 速度计算条件
                                {
                                        ...
                                        case 1:                      //按照每两个点构成的组进行进行个体速度计算
                                            {
                                                ...
                                                for (int si = 0; si < AllHumans[ahs].points.Count() - 1; si++)//计算该轨迹平均速度，两点构成的速度组数
                                                {
                                                    if ((AllHumans[ahs].points.Count() - 1) >= 16)  //Debug
                                                    {
                                                        //该跳过语句的判断条件是方法1速度筛选条件的非形式
                                                        if (((AllHumans[ahs].points.Count() - 1) / 2 - 4 > si) || ((AllHumans[ahs].points.Count() - 1) / 2 + 4 < si))  //如果组数大于16组，跳过未被选中的组
                                                        {
                                                            if (si != (AllHumans[ahs].points.Count() - 2))
                                                            {
                                                                continue;
                                                            }
                                                    ...
                                                    point = AllHumans[ahs].points[si];                            //提取轨迹点1进行计算
                                                    point2 = AllHumans[ahs].points[si + 1];                       //提取轨迹点2进行计算
                                                    deltDis = disCalculate.Dis(point, point2);                //距离计算                                                    
                                                    secHumanSpeed = deltDis / Parameters.timeDif;             //通过两轨迹点的速度m/s
                                                    if (si != (AllHumans[ahs].points.Count() - 2))
                                                    {
                                                        if ((AllHumans[ahs].points.Count() - 1) >= 16)            //Debug 筛选速度，速度组数大于等于阈值，选取速度不为0的组数（轨迹点数 - 1）/ 2组计算筛选速度
                                                        {
                                                            if (((AllHumans[ahs].points.Count() - 1) / 2 - 4 <= si) && ((AllHumans[ahs].points.Count() - 1) / 2 + 4 >= si))  //动态筛选轨迹中间一半的轨迹进行筛选速度的计算
                                                            {
                                                    ...
                                                    if ((AllHumans[ahs].points.Count() - 1) < 16)                 //Debug 不足组数按照常规方法求基础平均速度
                                                    {
                                                        if (secHumanSpeed != 0)
                                                        {
                                                ...
                                                if ((AllHumans[ahs].points.Count() - 1) >= 16)                    //Debug 满足组数要求，筛选速度替换基础速度
                                                {
                                                ...
                                                AllHumans[ahs] = human;                                           //当前行人信息更新  speed
                                        ...
                                        case 2:                                                               //按照首尾轨迹点构成的组进行个体速度计算
                                            {
                                                ...
                                                if (AllHumans[ahs].points.Count() >= 17)                          //轨迹点大于等于17个（对应法1的16组），筛选中间的轨迹点
                                                {
                                                    point = AllHumans[ahs].points[(AllHumans[ahs].points.Count() / 2) - (AllHumans[ahs].points.Count() / 4)];  //提取轨迹点1进行计算
                                                    point2 = AllHumans[ahs].points[(AllHumans[ahs].points.Count() / 2) + (AllHumans[ahs].points.Count() / 4)]; //提取轨迹点2进行计算
                                                    pointNum = 2 * (AllHumans[ahs].points.Count() / 4) + 1;    //实际的点数
                                                }
                                                else                                                       //点数低于17个选择首尾点
                                                {
                                                    point = AllHumans[ahs].points[0];                             //轨迹的第一个轨迹点
                                                    point2 = AllHumans[ahs].points[AllHumans[ahs].points.Count() - 1];//轨迹的最后一个轨迹点
                                                    pointNum = AllHumans[ahs].points.Count();                  //实际的点数
                                                }
                                                deltDis = disCalculate.Dis(point, point2);                 //距离计算
                    ...
                    else  //首次添加行人信息
                    {
                        ...
                        firstHuman.points = new List<Point>();
                        firstHuman.camid = human.camid;
                        firstHuman.id = human.id;
                        firstHuman.direction = human.direction;
                        firstHuman.flag = human.flag;
                        firstHuman.speed = human.speed;
                        for (int fpt = 0; fpt < human.points.Count(); fpt++ )
                        {
                            firstPoint.x = human.points[fpt].x;
                            firstPoint.y = human.points[fpt].y;
                            firstHuman.points.Add(firstPoint);
                        }
                        AllHumans.Add(firstHuman);
                    }
                }
            }
            ...
            if (Parameters.dataStatisticFrame == censusFrameNum)
            {
                ...
                for (; camNumber < sPipeInfo.iVideoCount; camNumber++)  //Debug  sPipeInfo.szVideoInfo.Length -》 sPipeInfo.iVideoCount
                {
                        ...
                        for (; aspd < AllSpeedData.Count(); aspd++)
                        {
                            ...
                            for (; aspdi < AllSpeedData[aspd].Count(); aspdi++)
                            {
                                ...
                                if (camNumID == AllSpeedData[aspd][aspdi].camid)  //匹配，速度存储为同摄像头存储
                                {
                                    ...
                                    breakSpeedFlag = true;  //跳出后续的比较循环，避免速度置位为0
                                }
                                else  //该路段的摄像头编号不匹配，跳过后续针对该编号摄像头数据信息的匹配
                                {
                                    break;
                                }
                            }
                    ...
                    //if (AllHumans.Count() > 0)
                    //{
                    //    for (int ah = 0; ah < AllHumans.Count(); ah++)
                    //    {
                    //        if (breakFlag)
                    //        {
                    //            break;
                    //        }
                    //        if (AllHumans[ah].Count() < 1)  //保证if(camNumID == AllHumans[ah][0].camid)中的下标0不越界
                    //        {
                    //            continue;
                    //        }

                    //        if (camNumID == AllHumans[ah][0].camid)
                    //        {
                    //            if (HumanAux.Count() > 0)
                    //            {
                    //                int ha = 0;
                    ...
                    //                        break;
                    //                    }
                    //                    if(HumanAux[ha].Count() < 1)
                    //                    {
                    //                        continue;
                    //                    }

                    //                    if(camNumID == HumanAux[ha][0].camid)
                    //                    {
                    //                        alterNumber = AllHumans[ah].Count(); //该摄像头在历史统计时间间隔保存的数据（行人信息）个数
                    //                        List<Human> NewHumans_AH = new List<Human>();  //针对新添加的行人
                    //                        List<Human> NewHumans_HAux = new List<Human>();
                    ...
                    //                                newHuman.points = new List<Point>();
                    //                                Human newHuman_Aux = new Human();
                    //                                newHuman_Aux.points = new List<Point>();
                    //                                if ((AllHumans[ah][an].id == HumanAux[ha][ani].id) && (AllHumans[ah][an].points.Count() > HumanAux[ha][ani].points.Count()))
                    //                                {
                    //                                    newHuman.camid = AllHumans[ah][an].camid;
                    //                                    newHuman.direction = AllHumans[ah][an].direction;
                    //                                    newHuman.flag = AllHumans[ah][an].flag;
                    //                                    newHuman.id = AllHumans[ah][an].id;
                    //                                    newHuman.speed = AllHumans[ah][an].speed;
                    //                                    Point newHumanPoint = new Point();
                    //                                    for (int nhpts = 0; nhpts < AllHumans[ah][an].points.Count(); nhpts++)
                    //                                    {
                    //                                        newHumanPoint.x = AllHumans[ah][an].points[nhpts].x;
                    //                                        newHumanPoint.y = AllHumans[ah][an].points[nhpts].y;
                    //                                        newHuman.points.Add(newHumanPoint);
                    //                                    }
                    //                                    NewHumans_AH.Add(newHuman);  //Humans[an] -》 human 或者采用从存储结构中直接添加的方式，即将该for循环所包括的信息全部替换为：humansaux.Add(Humans[an]);  Debug

                    //                                    newHuman_Aux.camid = AllHumans[ah][an].camid;
                    //                                    newHuman_Aux.direction = AllHumans[ah][an].direction;
                    //                                    newHuman_Aux.flag = AllHumans[ah][an].flag;
                    //                                    newHuman_Aux.id = AllHumans[ah][an].id;
                    //                                    newHuman_Aux.speed = AllHumans[ah][an].speed;
                    //                                    Point newHumanPointAux = new Point();
                    //                                    for (int nhptsa = 0; nhptsa < AllHumans[ah][an].points.Count(); nhptsa++)
                    //                                    {
                    //                                        newHumanPointAux.x = AllHumans[ah][an].points[nhptsa].x;
                    //                                        newHumanPointAux.y = AllHumans[ah][an].points[nhptsa].y;
                    //                                        newHuman_Aux.points.Add(newHumanPointAux);
                    //                                    }
                    //                                    NewHumans_HAux.Add(newHuman_Aux);  //Humans[an] -》 human 或者采用从存储结构中直接添加的方式，即将该for循环所包括的信息全部替换为：humansaux.Add(Humans[an]);  Debug
                    ...
                    //                        }
                    //                        AllHumans[ah].Clear();
                    //                        AllHumans[ah] = NewHumans_AH;  //跟新当前摄像头的行人信息，去除已经消失行人信息，保留新增的行人信息
                    //                        HumanAux[ha].Clear();
                    //                        HumanAux[ha] = NewHumans_HAux;  //辅助数据结构进行数据更新替换
                    //                        breakFlag = true;
                    //                        break;  //Same as 5.5.1
                    //                    }
                    //                    else
                    ...
                    //                    {
                    //                        Human newHumanAux = new Human();
                    //                        newHumanAux.points = new List<Point>();
                    //                        newHumanAux.camid = AllHumans[ah][an].camid;
                    //                        newHumanAux.direction = AllHumans[ah][an].direction;
                    //                        newHumanAux.flag = AllHumans[ah][an].flag;
                    //                        newHumanAux.id = AllHumans[ah][an].id;
                    //                        newHumanAux.speed = AllHumans[ah][an].speed;
                    //                        Point newPoint = new Point();
                    //                        for (int npts = 0; npts < AllHumans[ah][an].points.Count(); npts++)
                    //                        {
                    //                            newPoint.x = AllHumans[ah][an].points[npts].x;
                    //                            newPoint.y = AllHumans[ah][an].points[npts].y;
                    //                            newHumanAux.points.Add(newPoint);
                    //                        }
                    //                        NewHumansAux.Add(newHumanAux);  //Humans[an] -》 human 或者采用从存储结构中直接添加的方式，即将该for循环所包括的信息全部替换为：humansaux.Add(Humans[an]);  Debug
                    //                    }
                    //                    if(NewHumansAux.Count() > 0)
                    //                    {
                    ...
                    //                    Human firstHuman = new Human();
                    //                    firstHuman.points = new List<Point>();
                    //                    firstHuman.camid = AllHumans[ah][an].camid;
                    //                    firstHuman.direction = AllHumans[ah][an].direction;
                    //                    firstHuman.flag = AllHumans[ah][an].flag;
                    //                    firstHuman.id = AllHumans[ah][an].id;
                    //                    firstHuman.speed = AllHumans[ah][an].speed;
                    //                    Point firstPoint = new Point();
                    //                    for (int fsts = 0; fsts < AllHumans[ah][an].points.Count(); fsts++)
                    //                    {
                    //                        firstPoint.x = AllHumans[ah][an].points[fsts].x;
                    //                        firstPoint.y = AllHumans[ah][an].points[fsts].y;
                    //                        firstHuman.points.Add(firstPoint);
                    //                    }
                    //                    FirstHumansAux.Add(firstHuman);  //Humans[an] -》 human 或者采用从存储结构中直接添加的方式，即将该for循环所包括的信息全部替换为：humansaux.Add(Humans[an]);  Debug
                    //                }
                    //                if(FirstHumansAux.Count() > 0)
                    //                {
                ...
                //for (int cl = 0; cl < AllHumans.Count(); cl++)
                //{
                //    if (AllHumans[cl].points.Count < 1)
                //    {
                //        AllHumans.RemoveAt(cl);
                //        cl -= 1;
                //    }
                //    else
                //    {
                //        if (AllHumans[cl].points.Count() > 200)
                //        {
                //            AllHumans.RemoveAt(cl);
                //            cl -= 1;
                //        }
                //    }
                //}
                //for (int cl2 = 0; cl2 < HumanAux.Count(); cl2++)
                //{
                //    if (HumanAux[cl2].points.Count() < 1)
                //    {
                //        HumanAux.RemoveAt(cl2);
                //        cl2 -= 1;
                //    }
                //    else
                //    {
                //        if (HumanAux[cl2].points.Count() > 200)
                //        {
                //            HumanAux.RemoveAt(cl2);
                //            cl2 -= 1;
                //        }
                ...
                if(AllHumans.Count() > 0)
                {
                    if(HumanAux.Count() > 0)
                    {
                        for(int ah = 0; ah < AllHumans.Count(); ah++)
                        {
                            ...
                            {
                                if( (AllHumans[ah].camid == HumanAux[ha].camid) && (AllHumans[ah].id == HumanAux[ha].id) && (AllHumans[ah].points.Count() > HumanAux[ha].points.Count()) )
                                {
                                    for (int adpt = HumanAux[ha].points.Count(); adpt < AllHumans[ah].points.Count(); adpt++)
                                    {
                                        Point addNewPoint = new Point();
                                        addNewPoint.x = AllHumans[ah].points[adpt].x;
                                        addNewPoint.y = AllHumans[ah].points[adpt].y;
                                        HumanAux[ha].points.Add(addNewPoint);
                                    }
                                    breakFlag = true;
                                    break;
                                }
                                else if ((AllHumans[ah].camid == HumanAux[ha].camid) && (AllHumans[ah].id == HumanAux[ha].id) && (AllHumans[ah].points.Count() <= HumanAux[ha].points.Count()))
                                {
                                    AllHumans.RemoveAt(ah);
                                    ah -= 1;
                                    HumanAux.RemoveAt(ha);
                                    breakFlag = true;
                                    break;
                                }
                            }
                            if(!breakFlag)
                            {
                                if (ha >= HumanAux.Count())  //新添加
                                {
                                    Human newHumanAux = new Human();
                                    newHumanAux.points = new List<Point>();
                                    newHumanAux.camid = AllHumans[ah].camid;
                                    newHumanAux.id = AllHumans[ah].id;
                                    newHumanAux.direction = AllHumans[ah].direction;
                                    newHumanAux.flag = AllHumans[ah].flag;
                                    newHumanAux.speed = AllHumans[ah].speed;
                                    Point newHumanAuxPoint = new Point();
                                    for (int newpt = 0; newpt < AllHumans[ah].points.Count(); newpt++)
                                    {
                                        newHumanAuxPoint.x = AllHumans[ah].points[newpt].x;
                                        newHumanAuxPoint.y = AllHumans[ah].points[newpt].y;
                                        newHumanAux.points.Add(newHumanAuxPoint);
                                    }
                                    HumanAux.Add(newHumanAux);
                                }
                            }
                        }
                    }
                    ...
                    {
                        for(int fsthm = 0; fsthm < AllHumans.Count(); fsthm++)
                        {
                            Human firstHumanAux = new Human();
                            firstHumanAux.points = new List<Point>();
                            firstHumanAux.camid = AllHumans[fsthm].camid;
                            firstHumanAux.id = AllHumans[fsthm].id;
                            firstHumanAux.direction = AllHumans[fsthm].direction;
                            firstHumanAux.flag = AllHumans[fsthm].flag;
                            firstHumanAux.speed = AllHumans[fsthm].speed;
                            Point firstHumanAuxPoint = new Point();
                            for (int fstpt = 0; fstpt < AllHumans[fsthm].points.Count(); fstpt++)
                            {
                                firstHumanAuxPoint.x = AllHumans[fsthm].points[fstpt].x;
                                firstHumanAuxPoint.y = AllHumans[fsthm].points[fstpt].y;
                                firstHumanAux.points.Add(firstHumanAuxPoint);
                            }
                            HumanAux.Add(firstHumanAux);
                        }
                    }
                }
