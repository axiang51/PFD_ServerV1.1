...
namespace HST_Server
{
    ...
    public class ParamsCalculate
    {
        ...
        List<OutputData> AllOutputData = new List<OutputData>();//所有摄像头的输出数据，按照内部CamID区分
        ...
        public bool calculate(SHstPipeInfo sPipeInfo)  //进行计算的主要函数，数据输入格式为管道接收数据的格式
        {
            ...
            for (int i = 0; i < sPipeInfo.iVideoCount; i++)//Debug 第i个摄像头  修改为摄像头的实际数目IVideoCount
            {
                ...
                if (AllDensity.Count() > 0)
                {
                    ...
                    {
                        if (i == AllDensity[i].camid)
                        {
                            AllDensity[i].density.Add(sPipeInfo.szVideoInfo[i].iObjCount);
                        }
                    } 
                ...
                for (; j < sPipeInfo.szVideoInfo[i].iObjCount; j++)//第j个轨迹点
                {
                    ...
                    if(AllHumans.Count() > 0)  //存放所有人员的容器不为空
                    {
                        ...
                        {
                            Humans = AllHumans[ah];
                            if(i == Humans[0].camid)  //Debug 可能存在 Hunmans[0]不存在的情况；仅判断摄像头号码是否匹配，等价于i == AllHumans[ah][0].camid
                            {
                                Humans = AllHumans[i];    //令Humans指向AllHumans中的第i个摄像头所对应的行人信息，视作直接对AllHumans[i]进行操作，其后操作类似
                                int hi = 0;
                                        ...
                                        if(Humans[hi].points.Count() >= 10)  //Debug 进行人数统计的条件
                                        {
                                            if(-1 == human.flag)             //此前该轨迹未进行数量统计
                                            {
                                                ...
                                                if(AllHumansCensus.Count() > 0)
                                                {
                                                    ...
                                                    {
                                                        if (i == AllHumansCensus[hc].camid)  //查询到该路段保存的历史统计数据
                                                        {
            ...
            if(Parameters.dataStatisticFrame == censusFrameNum)
            {
                ...
                for (; camNumber < sPipeInfo.szVideoInfo.Length;camNumber++ )
                {
                    //5.1人数统计
